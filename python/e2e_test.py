...
config = SDKConfig(
    sidecar_addr=ADDR, tenant_id="acme", fail_mode="open", dev_mode=False
)
...
for path, expect_key, expect_val in [
    ...
    try:
        with urllib.request.urlopen(f"http://127.0.0.1:9091{path}", timeout=3) as r:
            ...
            check(
                f"GET {path}", body.get(expect_key) in ("ok", "ready"), json.dumps(body)
            )
...
check(
    "Any token → AuthDecision returned",
    isinstance(d.allowed, bool),
    f"allowed={d.allowed}  reason={d.reason!r}",
)
check(
    "Fail-open → allowed=True (no JWKS configured)", d.allowed, f"allowed={d.allowed}"
)

# Empty token
d2 = client.validate_token("")
check(
    "Empty token → still returns AuthDecision",
    isinstance(d2.allowed, bool),
...
check(
    "token + action=delete + resource=admin → allowed (fail-open)",
    d4.allowed,
...
for rule, inp in cases:
    ...
    check(
        f"Evaluate {rule} [{inp['subject']} / {inp['action']}]",
...
check(
    "PII fields redacted (email, Bearer, api_key)",
    pii_redacted,
    str({k: masked[k] for k in pii_fields}),
)
check(
    "Safe fields preserved (method, route, status)",
    safe_preserved,
...
check(
    "401 Unauthorized — correct status",
    err401.to_dict()["status"] == 401,
    err401.to_dict()["title"],
)
check(
    "403 Forbidden    — correct status",
    err403.to_dict()["status"] == 403,
    err403.to_dict()["title"],
)
check(
    "404 Not Found    — detail present",
    "doc-99" in err404.to_dict().get("detail", ""),
...
try:
    ...
    r2 = tc.get("/me")
    check(
        "FastAPI + real SDK → 401 without token", r2.status_code == 401, str(r2.json())
    )
