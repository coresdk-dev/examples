...
app.add_middleware(
    CoreSDKMiddleware, sdk=sdk, exclude_paths=["/healthz", "/docs", "/openapi.json"]
)
...
def current_user(request: Request) -> dict:
    ...
    return (
        ...
        else {"sub": "anonymous", "roles": [], "tenant_id": "unknown"}
    )
...
if __name__ == "__main__":
    ...
    check(
        "Valid token → 200 (fail-open)",
        r.status_code == 200,
        "sidecar allowed token (fail-open mode)",
    )
    ...
    check("Get doc-1", r.status_code == 200, f"title={r.json().get('title')}")

    r = client.get("/documents/doc-999", headers=ALICE)
    check(
        "Get unknown doc → 404 RFC 9457",
        r.status_code == 404,
    ...
    r = client.post(
        "/documents", json={"title": "New Doc", "content": "Hello"}, headers=ALICE
    )
    # With no JWKS, claims = {} → roles = [] → require_role("user") → 403
    check(
        "POST without roles → 403 (no roles in fail-open claims)",
    ...
    check(
        "DELETE without roles → 403 (no roles in fail-open claims)",
    ...
    check("10 concurrent GET /documents", ok == 10, f"{ok}/10 succeeded")
