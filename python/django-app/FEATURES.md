# Django App — CoreSDK Feature Coverage
...
> **Django note**: `CoreSDKMiddleware` stores JWT claims on `request.coresdk_claims` (a plain `dict`).
> Access the authenticated user with `request.coresdk_claims["sub"]`, tenant with
> `request.coresdk_claims["tenant_id"]`, and roles with `request.coresdk_claims["roles"]`.
...
| 1.4 | Config hot-reload | ✅ | `coresdk.toml` loaded via Django `settings.CORESDK`; all `sdk.*` keys overridable via `CORESDK_*` env vars |
...
| 1.6 | Tenant context propagation | ✅ | `request.coresdk_claims["tenant_id"]` → passed to every DB query and policy call |
...
| 1.8 | RFC 9457 error format | ✅ | All 401/403/404 return `application/problem+json` with `type` / `title` / `status` / `detail`. Django exception handler registered. |
...
| 1.15 | Distributed tracing (OTel Traces) | ✅ | `@trace(intent="list-products")` on every view creates OTel spans with W3C `traceparent` propagation. |
...
| 1.18 | Middleware framework | ✅ | `MIDDLEWARE = ["coresdk.django.CoreSDKMiddleware", ...]` in Django settings |
| 1.19 | RFC 9457 error propagation in SDK | ✅ | Django `EXCEPTION_HANDLER` converts all SDK errors to `problem+json` responses with `Content-Type: application/problem+json`. |
| 1.20 | Intent annotations (`@trace`) | ✅ | Every view: `@trace(intent="list-products")`, `@trace(intent="policy-check")`, etc. |
