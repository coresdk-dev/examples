...
for label, value, should_redact in cases:
    ...
    detail = "[REDACTED]" if redacted else f'"{result}"'
...
check(
    "mask_dict: name preserved",
    safe_profile["name"] == "Alice Smith",
    safe_profile["name"],
)
check(
    "mask_dict: email redacted",
    safe_profile["email"] == REDACTED,
    safe_profile["email"],
)
check("mask_dict: ssn redacted", safe_profile["ssn"] == REDACTED, safe_profile["ssn"])
check(
    "mask_dict: credit_card redacted",
    safe_profile["credit_card"] == REDACTED,
    safe_profile["credit_card"],
)
check(
    "mask_dict: notes (safe) preserved",
    safe_profile["notes"] == "Normal customer",
    safe_profile["notes"],
)
...
check(
    "mask_string: email replaced in text",
    "alice@company.com" not in safe_log,
    safe_log[:60],
)
check("mask_string: SSN replaced in text", "123-45-6789" not in safe_log, safe_log[:60])
check(
    "mask_string: surrounding text kept",
    "User" in safe_log and "called" in safe_log,
    safe_log[:60],
)
...
check(
    "mask_string: custom pattern redacted",
    "CUST-123456" not in safe_custom,
    safe_custom,
)
check(
    "mask_string: custom pattern: context kept", "Customer" in safe_custom, safe_custom
)
...
check(
    "mask_dict allowlist: tenant_id kept",
    safe_al["tenant_id"] == "acme-corp",
    safe_al["tenant_id"],
)
check(
    "mask_dict allowlist: order_id kept",
    safe_al["order_id"] == "ord-99",
    safe_al["order_id"],
)
check(
    "mask_dict allowlist: email redacted",
    safe_al["email"] == REDACTED,
    safe_al["email"],
)
