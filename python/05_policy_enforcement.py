...
print(
    f"{DIM}Load CORESDK_POLICY_DIR=/etc/coresdk/policy to enforce real Rego rules{RESET}\n"
)

print(f"{BOLD}RBAC — role-based actions{RESET}")
check(
    "alice  read  report",
    policy.can("alice", "read", "report", "acme"),
    sidecar_call=True,
)
check(
    "alice  delete report",
    policy.can("alice", "delete", "report", "acme"),
    sidecar_call=True,
)
check("bob    write  doc", policy.can("bob", "write", "doc", "acme"), sidecar_call=True)
check(
    "bob    delete doc", policy.can("bob", "delete", "doc", "acme"), sidecar_call=True
)
check(
    "carol  read  wiki", policy.can("carol", "read", "wiki", "acme"), sidecar_call=True
)
check(
    "carol  create doc", policy.can("carol", "create", "doc", "acme"), sidecar_call=True
)
check(
    "hacker read  anything",
    policy.can("hacker", "read", "anything", "acme"),
    sidecar_call=True,
)

print(f"\n{BOLD}ABAC — attribute-based (ownership context){RESET}")
check(
    "alice updates her own doc   (owner=alice)",
    policy.can("alice", "update", "doc-1", "acme", {"owner": "alice"}),
    sidecar_call=True,
)
check(
    "bob updates alice's doc     (owner=alice)",
    policy.can("bob", "update", "doc-1", "acme", {"owner": "alice"}),
    sidecar_call=True,
)
check(
    "alice updates bob's doc     (owner=bob)",
    policy.can("alice", "update", "doc-1", "acme", {"owner": "bob"}),
    sidecar_call=True,
)

print(f"\n{BOLD}Tenant isolation context{RESET}")
check(
    "alice reads acme resource   (resource_tenant=acme)",
    policy.can("alice", "read", "acme/data", "acme", {"resource_tenant": "acme"}),
    sidecar_call=True,
)
check(
    "alice reads globex resource (resource_tenant=globex)",
    policy.can("alice", "read", "globex/data", "acme", {"resource_tenant": "globex"}),
    sidecar_call=True,
)
...
check(
    "alice admin.billing",
    policy.can("alice", "admin.billing", "settings", "acme"),
    sidecar_call=True,
)
check(
    "bob   admin.billing",
    policy.can("bob", "admin.billing", "settings", "acme"),
    sidecar_call=True,
)
check(
    "alice admin.promote",
    policy.can("alice", "admin.promote", "users/carol", "acme"),
    sidecar_call=True,
)
...
for entry in policy.audit_log():
    ...
    print(
        f"  {colour}{'allow' if entry['allowed'] else 'deny ':5}{RESET}  "
